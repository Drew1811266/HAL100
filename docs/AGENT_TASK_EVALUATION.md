...
现有关键词路由的历史快照结果为6/20（30%）。迭代32新增四态结构化影子路由；当前v1扩展到
21个带提示词场景并由默认测试逐项校验，两个对抗场景均未形成任务。第19类模型停止和第20类
个人运行方案启用还分别由v8开放输入、v9动作纵向及独立纵向测试覆盖。新结果不参与
...
RPC v13已经接入真实Pi Agent Core提案边界。确定性路由为`Unresolved`时，Sidecar执行一次零工具
...
`contracts/agent-evals/v6-success-predicates.json`固定20/20类任务的成功谓词和允许的证据来源，
...
v8主集43场景与UI模板、v1和v3原句隔离，覆盖20/20任务、三类澄清、零工具解释、冲突和对抗。
确定性层当前33/43、越权任务0；9个安全长尾按设计交给Pi。独立Pi子集12场景×2轮只运行意图
...
v9当前以20条路径展开12类受控任务、11种原生动作、四类外部Agent的配置/断开和3类诊断修复
执行器。每条路径固定自然语言任务、Rust派生工具集合、允许动作、下层隔离执行验收与动作专属
复验证据；8类关键失败证据覆盖幂等、取消、过期、目标变化、执行失败、复验不满足/不可用和
回滚恢复。该矩阵达到20/20；每条引用的执行与失败验收都属于默认门禁，同一次完整检查实际运行
...
迭代46新增个人运行方案路径：Pi先读取有界运行目录，只能用Rust返回的精确方案ID生成一次性
启用计划；确认前不改变运行态。执行时Rust重新校验方案、模型、引擎版本与设备容量，随后复验
活动模型和容量，失败则回滚到切换前快照。记录见
[迭代46个人运行方案闭环](benchmarks/2026-08-26-iteration-46-runtime-profiles.md)。
