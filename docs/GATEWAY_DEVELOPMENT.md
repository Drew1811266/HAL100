...
- `hal100-active`始终选择当前活动后端；没有显式别名的其他模型名也走活动后端并保持原模型ID。命中显式别名时，Gateway选择指定后端并只重写请求中的`model`字段。
...
- 非敏感后端与别名由SQLite schema v7恢复。Keychain凭据缺失、无效或不可读时，该后端和引用它的运行态别名不会加载，界面显示“凭据不可用”。
...
- “发现本机服务”只并发探测`127.0.0.1:11434`、`:8000`和`:8080`的固定已知入口，每个入口1秒总超时、1 MiB响应上限。不会枚举进程、扫描端口范围或访问局域网。

## 4. 当前不包含

- 跨协议转换、Token估算和每日聚合。
- 局域网广播发现、任意端口扫描和更完整的后端能力矩阵。
- OpenCode配置链路已进入迭代3并由[OpenCode集成开发说明](OPENCODE_INTEGRATION.md)单独记录，不再属于本迭代2缺项。
- 云端Agent Provider配置仍属于迭代8；本阶段Keychain只用于用户登记的推理后端凭据。

云端Agent Provider能力进入迭代8；OpenCode、通用本地客户端与托管 llama.cpp的现有纵向能力由各自开发说明和迭代6验收记录覆盖。
