...
该目录是 Rust Core 与 Agent Kernel Sidecar 的稳定私有协议说明。当前v3使用四字节大端长度前缀加UTF-8 JSON；单帧最大1 MiB。v3增加按需环境诊断与受控单项修复计划能力，不兼容v2。
...
Rust同时验证run ID、tool call ID、重复调用、调用顺序和每任务最多4次工具调用。`agent.run.completed`必须报告固定的10个注册工具、实际完成工具名与数量；每项任务最多一个可写计划。必需工具缺失、计划类型错误、回答为空/过长或任何关联不一致均拒绝整个任务。
...
Provider每一回合只获得当前缺失的一个工具定义并使用`tool_choice: "required"`；完成后才暴露下一工具。若模型在成功工具结果后提前结束，Sidecar最多在同一Pi会话追加两次固定纠偏提示。该机制不执行工具、不能扩大白名单；固定认证、路由和连接错误不会进入纠偏重试。
...
取消由Rust进程外控制：活动run持有原子取消标记，模型哈希每1 MiB、健康等待每50 ms、RPC接收每100 ms检查；取消后父进程终止Sidecar并回收模型和临时资源，不向不可信Sidecar请求“同意取消”。
