...
const VLLM_API_ROOT: &str = "http://127.0.0.1:8000/v1/";

pub fn vllm_agent_protocol_capabilities() -> EngineProtocolCapabilitySet {
    protocol_capability_set([
...
pub fn vllm_agent_protocol_capability_hash() -> String {
    protocol_capability_hash(&vllm_agent_protocol_capabilities())
}

/// Read-only vLLM contract probe.
///
/// Iteration 51D intentionally registers only `Connected` support units. This adapter proves that
/// the common target/observation/evidence path works for a second engine without making vLLM
/// selectable or saveable as formally supported before real Linux/CUDA qualification.
#[derive(Clone)]
pub struct VllmExternalEngineAdapter {
    http: BoundedEngineHttpClient,
    qualification_http: BoundedEngineHttpClient,
}

impl VllmExternalEngineAdapter {
    pub fn new() -> Result<Self, ExternalEngineAdapterError> {
        Ok(Self {
            http: BoundedEngineHttpClient::new("vllm-read-only-adapter").map_err(map_http_error)?,
            qualification_http: BoundedEngineHttpClient::with_timeouts(
                "vllm-qualification-adapter",
    ...
    async fn inspect_inner(
        ...
        Ok(ExternalEngineSnapshot {
            ...
            display_name: "用户所有的本机 vLLM".to_owned(),
    ...
    async fn qualify_inner(
        ...
        // Carry the exact server version into the qualification report as well as the
        // read-only inspection snapshot. Runtime profile lifecycle checks compare these
        // two observations to detect a server replacement between save and activation.
        let observed_engine_version = parse_version(
            &self
                .qualification_http
                .get_bounded(target, "/version", MAX_VERSION_BODY_BYTES)
                .await
        ...
        let protocol_capabilities = vllm_agent_protocol_capabilities();
...
impl EngineInspector for VllmExternalEngineAdapter {
    ...
    fn protocol_capability_hash(&self) -> Option<String> {
        Some(vllm_agent_protocol_capability_hash())
    }

    fn default_target(&self) -> Option<VerifiedEngineTarget> {
        VerifiedEngineTarget::external_local("discovery:vllm", &self.manifest(), VLLM_API_ROOT, 0)
            .ok()
    }
...
impl ExternalInferenceEngineAdapter for VllmExternalEngineAdapter {}
...
mod tests {
    ...
    #[tokio::test]
    async fn vllm_adapter_inspects_and_qualifies_the_bounded_agent_protocol_contract() {
        let app = Router::new()
            .route(
                "/version",
                get(|| async { axum::Json(json!({"version":"0.10.2"})) }),
            )
            .route("/health", get(|| async { "ok" }))
            .route(
                "/v1/models",
                get(|| async { axum::Json(json!({"data":[{"id":"Qwen/Qwen3-8B"}]})) }),
            )
            .route(
                "/v1/chat/completions",
        ...
        let adapter = VllmExternalEngineAdapter::new().expect("adapter");
        ...
        assert_eq!(snapshot.engine, InferenceEngineKind::Vllm);
        assert_eq!(snapshot.version, "0.10.2");
        assert_eq!(snapshot.models.len(), 1);
        assert_eq!(
            snapshot.models[0].evidence.kind,
            RuntimeProfileEvidenceKind::CatalogIdentity
        );
        assert_eq!(snapshot.models[0].evidence.value, "Qwen/Qwen3-8B");
        assert_eq!(snapshot.models[0].digest.len(), 64);
        let qualification = adapter
            .qualify(&target, "Qwen/Qwen3-8B")
            .await
            .expect("qualification");
        assert_eq!(qualification.model_id, "Qwen/Qwen3-8B");
        assert_eq!(
        ...
        assert_eq!(qualification.protocol_capability_hash.len(), 64);
        assert_eq!(
            qualification.protocol_capabilities.capabilities,
            vec![
                EngineProtocolCapability::ModelsList,
                EngineProtocolCapability::ChatCompletionsUnary,
                EngineProtocolCapability::ChatCompletionsStream,
